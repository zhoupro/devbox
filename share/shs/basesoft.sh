...
function install_node_server(){
    ...
    if [ ! -f  node-v${SERVER_VERSION}-linux-x64.tar.xz ];then
        ...
        sudo axel -n 6 -o node-v${SERVER_VERSION}-linux-x64.tar.xz https://nodejs.org/dist/v${SERVER_VERSION}/node-v${SERVER_VERSION}-linux-x64.tar.xz && \
        ...
        sudo echo "export PATH=\$PATH:/usr/local/bin/nodejs/node/bin" >> /home/vagrant/.zshrc
        sudo echo "export PATH=\$PATH:/usr/local/bin/nodejs/node/bin" >> /home/vagrant/.bashrc
    fi
}
...
if [ ! -f /usr/local/bin/w2 ];then
    sudo /usr/local/lib/nodejs/node/bin/npm install whistle -g --registry=https://registry.npm.taobao.org
...
sudo cp /vagrant_data/conf/rootCA.crt /usr/local/share/ca-certificates/
...
function install_go_server(){
    ...
    sudo echo "export PATH=\$PATH:/usr/local/go/bin:$HOME/go/bin" >> /home/vagrant/.bashrc
}

install_go_server 1.17.7

proxy
sudo -E -H -u vagrant bash /vagrant_data/shs/zsh.sh
noproxy
